...
class AttentionModel(NeuralModel):
    ...
    def setup_backprop(self):
        ...
        # LM
        # TODO: add LM backprop
        nll_lm, p_x_seq_lm, p_y_seq_lm, objective_lm, updates_lm = self._setup_backprop_lm(x, y, eta, l2_reg)
        self._get_nll_lm = theano.function(inputs=[x, y], outputs=nll_lm, on_unused_input='warn')
        self._backprop_lm = theano.function(inputs=[x, y, eta, l2_reg], outputs=[p_x_seq_lm, p_y_seq_lm, objective_lm],
                                            updates=updates_lm)
    ...
    def _setup_backprop_lm(self, x, y, eta, l2_reg):
        # setup both encoder and decoder LMs
        # TODO: write this
        ...
        dec_results, _ = theano.scan(
            ...
            outputs_info=[dec_init_state, None],  # initialization
            non_sequences=[annotations] + self.spec.get_all_shared())    # unchanging variables
        ...
        return nll_lm, p_x_seq_lm, p_y_seq_lm, objective_lm, updates_lm
        # TODO: write this

    def _setup_backprop_with(self, dec_init_state, annotations, y, y_in_x_inds, eta, l2_reg):
        def decoder_recurrence(y_t, cur_y_in_x_inds, h_prev, annotations, *params):
            # get_h_for_write unpacked c_h_t into -> (c_t, h_t) and returns h_t
            h_for_write = self.spec.decoder.get_h_for_write(h_prev)
            # h_for_write is s_t (or s_j), annotations is b, this produces e
            scores = self.spec.get_attention_scores(h_for_write, annotations)
            # softmax of scores to produce alpha
            alpha = self.spec.get_alpha(scores)
            # calculate c_j based on alpha and b
            c_t = self.spec.get_context(alpha, annotations)
            # get the softmax using s_t, c_t, and
            # concatenates h_t and c_t into input_t and then
            write_dist = self.spec.f_write(h_for_write, c_t, scores)
            ...
            # do an LSTM step to get the next state, gets the embedding of y_t (correct output)
            # concatenates y_t_embed with c_t into input_t and does an LSTM step with (input_t, h_prev)
            h_t = self.spec.f_dec(y_t, c_t, h_prev)
            return h_t, p_y_t

        # sending outputs_info + sequences +
        dec_results, _ = theano.scan(
            fn=decoder_recurrence, sequences=[y, y_in_x_inds],  # the sequence to loop over
            outputs_info=[dec_init_state, None],  # initialization
            non_sequences=[annotations] + self.spec.get_all_shared())  # unchanging variables
        p_y_seq = dec_results[1]
        log_p_y = T.sum(T.log(p_y_seq))
