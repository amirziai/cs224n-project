...
class NeuralModel(object):
    ...
    def sgd_step_lm(self, ex, eta, l2_reg):
        return self._backprop_lm(ex.x_inds, ex.y_inds, eta, ex.y_in_x_inds, l2_reg)
    ...
    def train(self, dataset, eta=0.1, T=[], verbose=False, dev_data=None,
        ...
        # Stage 1
        # train independent Language Models (LM)
        for it in range(max_iters):
            ...
            # SGD step per training example
            for ex in cur_dataset:
                nll = self.sgd_step_lm(ex, cur_lr, l2_reg)
                total_nll += nll
            ...
            if dev_data:
                for ex in dev_data:
                    # the sum of NLL for enc and decoder
                    dev_nll += self._get_nll_lm(ex.x_inds, ex.y_inds, ex.y_in_x_inds)
            if DEBUG:
                self.on_train_epoch(it)
            t1 = time.time()
            print 'LM NeuralModel.train(): iter %d (lr = %g): train obj = %g, dev nll = %g (%g seconds)' % (
                it, cur_lr, total_nll, dev_nll, t1 - t0)

        # Stage 2
        # train with SGD (batch size = 1)
        # uses seq2seq objective function
        # TODO: add joint learning
        ...
        for it in range(max_iters):
            ...
            for ex in cur_dataset:
                ...
                if do_distract:
                    ...
                    _ = self.sgd_step(ex, cur_lr, l2_reg, distractors=distractors)
                else:
