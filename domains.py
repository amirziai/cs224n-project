...
class GeoqueryDomain(Domain):
    ...
    def print_failures(self, dens, name):
        ...
        print('%s: %d syntax errors, %d executor errors' % (name, num_syntax_error, num_exec_error))
    ...
    def compare_answers(self, true_answers, all_derivs):
        all_lfs = ([self.format_lf(s) for s in true_answers] +
                   [self.format_lf(' '.join(d.value))
                    for x in all_derivs for d in x])
        tf_lines = ['_parse([query], %s).' % lf for lf in all_lfs]
        tf = tempfile.NamedTemporaryFile(suffix='.dlog', mode='w')
    ...
    def compare_answers(self, true_answers, all_derivs):
        all_lfs = ([self.format_lf(s) for s in true_answers] +
                   [self.format_lf(' '.join(d.value))
                    for x in all_derivs for d in x])
        tf_lines = all_lfs
        tf = tempfile.NamedTemporaryFile(suffix='.examples', mode='w')
        for line in tf_lines:
            print(line, file=tf)
            # print(line)
        tf.flush()
