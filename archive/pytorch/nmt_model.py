...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.encoder = nn.LSTM(input_size=self.model_embeddings.embed_size, hidden_size=self.hidden_size,
    ...
    def encode(self, source_padded: torch.Tensor,
        ...
        #         - `init_decoder_hidden`:
        #             `last_hidden` is a tensor shape (2, b, h). The first dimension corresponds to forwards and bwd.
        #             Concatenate the forwards and backwards tensors to obtain a tensor shape (b, 2*h).
        #             Apply the h_projection layer to this in order to compute init_decoder_hidden.
        #             This is h_0^{dec} in the PDF. Here b = batch size, h = hidden size
        #         - `init_decoder_cell`:
        #             `last_cell` is a tensor shape (2, b, h). The first dimension corresponds to forwards and bwd.
        #             Concatenate the forwards and backwards tensors to obtain a tensor shape (b, 2*h).
        ...
        out, (last_hidden, last_cell) = self.encoder(pack_padded_sequence(X, source_lengths))
        enc_hiddens = pad_packed_sequence(out)[0].permute(1, 0, 2)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        #         - To get the tensors into the right shapes for bmm, you will need to do some squeezing/unsqueezing.
        #         - When using the squeeze() function make sure to specify the dimension you want to squeeze
        #             over. Otherwise, you will remove the batch dimension accidentally, if batch_size = 1.
        #
        # Use the following docs to implement this functionality:
        ...
        dec_hidden_permuted = dec_hidden_unsqueeze.permute(0, 2, 1)
        enc_hiddens_proj_permuted = enc_hiddens_proj.permute(0, 2, 1)
        e_t_to_be_squeezed = torch.bmm(dec_hidden_permuted, enc_hiddens_proj_permuted)
        e_t = torch.squeeze(e_t_to_be_squeezed, dim=1)
        ...
        alpha_t_unsqueezed = torch.unsqueeze(alpha_t, dim=2)
        alpha_t_permuted = alpha_t_unsqueezed.permute(0, 2, 1)
        a_t_to_be_squeezed = torch.bmm(alpha_t_permuted, enc_hiddens)
        a_t = torch.squeeze(a_t_to_be_squeezed, dim=1)
